...
def execute_GPT3_few_shot(courses, questions_per, embeddings_location):
    """
    ...
    for course_index, course in enumerate(courses):
        ...
        for i in range(questions_per):
            ...
            rows.append([question, original_question, question_answer,
                         gpt3_input, gpt3_output, most_sim_q_index+1])
        info = pd.DataFrame(rows, columns=['Question', 'Original Question', 'Actual Solution',
                                           'GPT-3 Input', 'GPT-3 Output', 'Most Similar Question'])
        course_results_location = course + ' GPT3_few_shot_results.csv'
        info.to_csv(course_results_location, index=False)

if __name__ == "__main__":
    #zero-shot step for courses:
    execute_GPT3_few_shot(courses_to_zero_shot, questions_per_course, courses_embeddings_location)
    #zero-shot step for MATH benchmark:
    execute_GPT3_few_shot(MATH_sections_to_zero_shot, questions_per_MATH_section, MATH_embeddings_location)
