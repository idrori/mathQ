...
plot.mit <- d.mit_long %>% ggplot(aes(Course, Accuracy, fill = Model)) +
  ...
  geom_errorbar(aes(x = 8 - 0.2, y = mean, ymin = mean - se, ymax = mean + se), width = 0.1, position = position_dodge(-0.2), data = d.mit_se %>% filter(grepl("GPT-3", Model))) +
...
plot.math <- d.math_long %>% ggplot(aes(Topic, Accuracy, fill = Model)) +
  ...
  geom_errorbar(aes(x = 7 - 0.2, y = mean, ymin = mean - se, ymax = mean + se), width = 0.1, position = position_dodge(-0.2), data = d.math_se %>% filter(grepl("GPT-3", Model))) +
